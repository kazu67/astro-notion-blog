...
export const getStaticFilePath = (path: string): string => pathJoin(BASE_PATH, path);
...
export const getPostLink = (slug: string) => pathJoin(BASE_PATH, `/posts/${slug}`);

export const getTagLink = (tag: string) => pathJoin(BASE_PATH, `/posts/tag/${encodeURIComponent(tag)}`);
...
export const buildHeadingId = (heading: Heading1 | Heading2 | Heading3) => heading
  .RichTexts
